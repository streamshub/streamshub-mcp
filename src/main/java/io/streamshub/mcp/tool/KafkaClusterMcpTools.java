...
public class KafkaClusterMcpTools {
    ...
    public Object getKafkaClusterPods(
        @ToolArg(description = StrimziToolsPrompts.MCP_NAMESPACE_PARAM_DESC)
        String namespace,
        @ToolArg(description = StrimziToolsPrompts.MCP_CLUSTER_NAME_PARAM_DESC)
        String clusterName
    ) {
        try {
    ...
    /**
     * Get detailed information for a specific Kafka topic.
     *
     * @param namespace   namespace to search or null for auto-discovery across all namespaces
     * @param clusterName name of the Kafka cluster (optional, helps validate topic belongs to specific cluster)
     * @param topicName   name of the specific topic to get details for
     * @return detailed topic information or error details
     */
    @Tool(
        name = "strimzi_topic_details",
        description = "Get detailed information for a specific Kafka topic by name. " +
            "Retrieves comprehensive topic configuration including partitions, replicas, status, and cluster association. " +
            "If namespace is not specified, searches across all namespaces to find the topic. " +
            "If clusterName is specified, validates the topic belongs to that specific cluster." +
            StrimziToolsPrompts.MCP_AUTO_DISCOVERY_GUIDANCE
    )
    public Object getTopicDetails(
        @ToolArg(description = StrimziToolsPrompts.MCP_NAMESPACE_PARAM_DESC)
        String namespace,
        @ToolArg(description = StrimziToolsPrompts.MCP_CLUSTER_NAME_PARAM_DESC)
        String clusterName,
        @ToolArg(description = "Name of the specific topic to get details for (e.g., 'user-events', 'orders'). Required.")
        String topicName
    ) {
        try {
            return topicService.getTopicDetails(namespace, clusterName, topicName);
        } catch (Exception e) {
            return ToolError.of("Failed to retrieve topic details", e);
        }
    }
}
