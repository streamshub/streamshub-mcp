...
import io.streamshub.mcp.dto.KafkaNodePoolResponse;
...
public class KafkaService {
    ...
    private KafkaClusterResponse createClusterResponse(final Kafka kafka) {
        ...
        if (kafka.getStatus() != null) {
            status = k8sService.determineResourceStatus(kafka.getStatus().getConditions());
        } else {
            status = Constants.Kubernetes.ResourceStatus.UNKNOWN;
        }
    ...
    private String extractManagedBy(final Kafka kafka) {
        ...
        return labels != null ? labels.get(Constants.Kubernetes.Labels.MANAGED_BY) : null;
    }
        ...
        try {
            List<Pod> kafkaPods = k8sService.queryResourcesByLabel(
                ...
                .filter(pod -> pod.getMetadata().getLabels() != null
                    && Constants.Strimzi.ComponentTypes.KAFKA.equals(
            ...
            return (int) kafkaPods.stream()
                .filter(pod -> pod.getStatus() != null
                    ...
                    .anyMatch(cond -> Constants.Kubernetes.Conditions.TYPE_READY.equals(cond.getType())
                        && Constants.Kubernetes.Conditions.STATUS_TRUE.equals(cond.getStatus())))
                .count();
        } catch (Exception e) {
    ...
    private String extractStorageType(final Kafka kafka) {
        ...
        try {
            ...
            return nodePools.stream()
                ...
                .filter(type -> type != null && !type.equals(Constants.UNKNOWN))
                .findFirst()
                .orElse(null);
        } catch (Exception e) {
