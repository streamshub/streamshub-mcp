...
- **Deterministic `tools/list` ordering regression test** (#265) — `McpDiscoveryTest.testToolListingOrderIsDeterministic` pins the framework guarantee that `tools/list` returns tools in a stable order across consecutive requests (MCP 2026-07-28 recommendation).
