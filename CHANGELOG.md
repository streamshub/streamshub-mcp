...
- **Shared coding agent skills** -- Added `.agents/skills/` directory with 6 reusable skills (code review, test coverage, Java best practices, add MCP tool, add diagnostic tool, add system test) consumed by Claude Code, Bob, Cursor, GitHub Copilot, and Windsurf via symlinks or agent-specific references
