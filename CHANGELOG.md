...
- **Throughput metrics were the mean of a total and its own parts** — Kafka publishes `BrokerTopicMetrics` twice, once per topic and once as a broker-wide roll-up with no `topic` label: eleven `kafka_server_brokertopicmetrics_bytesin_total` series on a single dev broker. The `throughput` category defaults to `CLUSTER`, which strips `topic`, so all eleven shared a group key and `get_kafka_metrics(category=throughput)` returned their average — a total averaged with its own constituents, which is not a quantity. `AggregatedTimeSeries` now prefers the roll-up: when a group holds both samples that carry a stripped dimension and samples that do not, only the latter are combined. Groups where every sample carries the dimension, or none does (`pod`, which every scraped sample has), are untouched. Response size is unchanged — those series already collapsed into one row — but `source_count` now reports the real number of source series.
- **Throughput was reported per broker instead of cluster-wide** — with the roll-up selected, the seven `brokertopicmetrics` counters and `connection_count` now `SUM` across brokers, so the default response is the cluster's actual ingress/egress rate rather than one broker's share, tagged `aggregation_fn` = "sum". `aggregation=broker` still gives the per-broker view for spotting imbalance, and `aggregation=topic` the per-topic breakdown; the interpretation text now says so, and warns that the topic-level response carries one row without a `topic` label which is the total of the others.
- **`MetricAggregation` entries were unreachable for any renamed metric** — the table is written in JMX catalog spelling but consulted with the name as it appears in the response, after the Prometheus provider renames counters from `_total` to `_rate_per_second` and after the SMR alias translation. The seven original entries worked only because none of them is renamed; every throughput metric is renamed *twice*. `forMetric` now retries an exact miss under the counter's pre-rename name, the `_total`/`_rate_per_second` pair moved into a shared `MetricNameSuffixes` (it was duplicated in `PrometheusMetricsProvider` and `MetricNameResolver`), SMR spellings are listed alongside their JMX counterparts, and a new test fails the build if an aliased metric gains a non-default aggregation without its SMR name.
- **Aggregation was chosen for Kafka broker metrics only** — Connect worker metrics (`connector_count`, `task_count`, the four startup counters) are per-worker MBeans in a category pinned to `CLUSTER`, so a multi-worker cluster reported the mean per worker where the caller reads a cluster count; `kafka_consumergroup_lag` averaged per partition instead of totalling the group's backlog; additive Bridge counters averaged across replicas, whose `clientId` labels are identical and therefore do collapse. All now `SUM`. Strimzi operator metrics needed nothing — `kind` and `namespace` survive every level, so those series never share a group.
- **Four catalog metric names did not exist on any deployment** — the golden-file regeneration flagged five names that never appear in a scrape; checking them against the upstream sources showed four were simply wrong. `strimzi_bridge_http_server_requestCount_total` is Vert.x 3 Micrometer spelling — the bridge runs Vert.x 5, which emits `..._requests_total`. `strimzi_bridge_http_server_request_size_bytes` and `..._response_size_bytes` have never existed in the bridge at all (`git log -S"size_bytes"` on strimzi-kafka-bridge returns nothing); the real series are the Micrometer summary triples `..._{request,response}_bytes_{count,sum,max}`, now all six in the `http` category with `count`/`sum` summing and `max` taking the maximum across replicas. `kafka_consumergroup_lag_seconds` is not published by Kafka Exporter, so the `consumer_lag` guide promised an "estimated time in seconds to catch up" that no backend could answer; the name is dropped and the guide now states the unit is messages and how to derive catch-up time.
- **No bridge metrics reached Prometheus on the dev cluster** — `dev/manifests/strimzi/kafka-bridge/040-PodMonitor.yaml` scraped `/metrics` on port `rest-api` (8080), but the bridge serves its exposition on `rest-api-mgmt` (8081); 8080 returns an empty body. Every `get_kafka_bridge_metrics` call therefore came back with zero samples regardless of category. Found while confirming the renamed HTTP metric names by scraping the pod directly.
- **`strimzi_resource_state` is absent on Strimzi 1.2.0+** — not a wrong name but version drift: added by strimzi-kafka-operator #3652 and removed as deprecated by #13025, first absent in 1.2.0. Kept in the `resources` category for clusters still on 1.1.x; its description and the operator-metrics prompt now say it disappears on 1.2.0 and point at the resource's own status conditions instead, so an empty result is not read as a fault.
