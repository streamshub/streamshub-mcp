...
/**
 * Rate limit guardrail for log tools.
 *
 * <p>Enforces a configurable requests-per-minute limit on log tool calls.
 * Configured via {@code mcp.guardrail.rate-limit.log-rpm}. Default is 0 (disabled).</p>
 */
@SupportedExecutionModels({ExecutionModel.WORKER_THREAD, ExecutionModel.VIRTUAL_THREAD})
@Singleton
public class LogRateLimitGuardrail extends AbstractRateLimitGuardrail {

    @ConfigProperty(name = "mcp.guardrail.rate-limit.log-rpm", defaultValue = "0")
    int rpm;

    LogRateLimitGuardrail() {
    }
